...
gradeController.studentGetGradeBySemester = async (req, res, next) => {
    try {
        ...
        const studentGrade = await gradeServices.studentGetGradeBySemester(studentId, semester)
...
gradeController.teacherAddScore = async (req, res, next) => {
    try {
        ...
        const updatedTotalScore = await gradeServices.teacherAddScore(studentId, courseId, semester, type, point);
...
gradeController.teacherEditScore = async (req, res, next) => {
    try {
        ...
        if (!(type || point)) {
            return res.status(400).json({ error: 'At least one field is require change' });
        }
        ...
        const editScore = await gradeServices.teacherEditScore(componentId, updatedData);
