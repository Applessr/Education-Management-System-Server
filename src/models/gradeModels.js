  ...
  for (const semester of semesters) {
    try {
      const { semesterGPA, semesterCredits } =
        await gradeModels.studentGetGPABySemesterForAll(
          studentId,
          semester.semester
        );
      ...
      if (semesterGPA == null || semesterCredits == null) {
        console.log(
          `No GPA or credits found for semester ${semester.semester} for student ${studentId}`
        );
        continue;
      }

      console.log(
        `Semester: ${semester.semester}, GPA: ${semesterGPA}, Credits: ${semesterCredits}`
      );
      ...
      console.error(
        `Error fetching GPA for semester ${semester.semester} for student ${studentId}:`,
        error
      );
    }
  }
...
gradeModels.studentGetGPABySemesterForAll = async (studentId, semester) => {
  ...
  for (const enrollment of enrollments) {
    ...
    console.log(
      `Grades for Course ID: ${enrollment.courseId}`,
    ...
    if (grade && grade.letterGrade !== null) {
      ...
      if (gradePoint !== undefined) {
        ...
        console.log(
          `Course ID: ${enrollment.courseId}, Letter Grade: ${letterGrade}, Grade Points: ${gradePoint}, Credits: ${credits}`
        );
      } else {
        console.log(
          `Invalid letter grade for Course ID: ${enrollment.courseId}`
        );
      }
    } else {
  ...
  console.log(
    `Total Grade Points: ${totalGradePoints}, Total Credits: ${totalCredits}, Semester GPA: ${semesterGPA}`
  );
...
gradeModels.teacherAddScore = async (
  studentId,
  courseId,
  semester,
  type,
  ...
  const gradeId = await gradeModels.findOrCreateGrade(
    studentId,
    courseId,
    semester
  );
...
gradeModels.teacherEditScore = async (componentId, updatedData) => {
  ...
  const updatedTotal = await gradeModels.updateTotalScore(
    gradeComponent.gradeId
  );
...
gradeModels.studentGetAllGrade = async (studentId) => {
  return await prisma.grade.findMany({
    where: {
      studentId: Number(studentId),
    },
